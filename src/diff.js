// Version:3.0.0
import { state, useTemplate } from './init';
import { getType, checkVnode, isHasFlag, isComplexType, isXlink, isSameObject, isVnode, xlinkNS, setStyleProp, addEvent, removeEvent, createNode, useFragmentNode, } from './util';
...
function patch(n1, n2, status) {
    ...
    if (isHasFlag(oldProps, flag[0]) && n1.tag !== n2.tag && n1.el) {
        ...
        let el = null;
        if (isHasFlag(oldProps, flag[0]) && n1.el) {
        ...
        return Promise.resolve()
            ...
            if (state._el) {
                if (options && options.status === 'useRouter') {
                    state._el.innerHTML = '';
                    typeof state._template === 'function' &&
